...
bool opt_ccache_force_cpu_id = false;  /* $$$  cid addition  $$$ */
...
/* $$$  cid addition  $$$ */
/* PROBLEM: Currently, the libc-header DOESN'T INCLUDE the "new fields"  (which are already "supported" by kernel versions 6.3.0+)
 * THUS: WORKAROUND: Use type from header in git linux repo  (https://raw.githubusercontent.com/torvalds/linux/master/include/uapi/linux/rseq.h )
 *     - TODO: 2 be RMVed later (once compatible header available)  !!!!
 * SEE ALSO: https://github.com/torvalds/linux/tree/master/tools/testing/selftests/rseq
 */
#include <linux/types.h>
struct rseq_abi__workaround {
    __u32 cpu_id_start;
    __u32 cpu_id;
    __u64 rseq_cs;
    __u32 flags;
    // >>>  BELOW = NEW FIELDS (which are missing in the current header files) ...  <<<
    __u32 node_id;
    __u32 mm_cid;
    char end[];
...
// Derived from librseq       /* $$$  cid addition  $$$ */
#include <sys/auxv.h>
...
#  endif
    ...
    const unsigned long auxv_rseq_align = getauxval(AT_RSEQ_ALIGN);
...
#endif
    return (int)(auxv_rseq_feature_size ?  : ORIG_RSEQ_FEATURE_SIZE) >= (int)rseq_offsetofend(struct rseq_abi__workaround, mm_cid);
}
...
static void *
ccache_bin_finish_fill(ccache_bin_t *bin, cache_bin_ptr_array_t *arr, cache_bin_sz_t nfilled) {
...
static void *
ccache_bin_refill(tsd_t *tsd, ccache_t *ccache, ccache_bin_t *bin, arena_t *arena, szind_t binind, bool small) {
...
static bool
ccache_bin_flush(tsd_t *tsd, ccache_t *ccache, ccache_bin_t *bin, arena_t *arena, szind_t binind, void *ptr, bool small) {
...
static void
ccache_bin_full_flush_unsafe(tsd_t *tsd, ccache_bin_t *bin, szind_t binind, bool small) {
...
ccache_init(tsdn_t *tsdn, base_t *base) {
	...
	if (!opt_ccache_force_cpu_id  &&  !rseq_mm_cid_available()) {  /* $$$  cid addition  $$$ */
#if defined(JEMALLOC_DEBUG)
		malloc_printf("<jemalloc>: Current kernel doesn't support `mm_cid`, using CPU id as fallback");
#endif
		opt_ccache_force_cpu_id = true;
	}
...
void*
ccache_alloc(tsd_t *tsd, arena_t *arena, size_t size, szind_t ind, bool zero, bool small) {
	assert(opt_ccache);
	assert(ccache_handles_szind(ind));
	...
	size_t bin_offset = (ind - ccache_minind) * sizeof(ccache_bin_t) +  offsetof(ccache_t, bins);
	...
	__asm__ __volatile__(
	    ...
	    "sete %[refill_flag]\n\t"
	    "je refill_needed%=\n\t"
	    /* Store allocation in ret and prepare head++ */
	    "movq (%%rax), %[ret]\n\t"
	    "addq $8, %%rax\n\t"
	    "jmp commit%=\n\t"
	    ...
	    : [bin_offset] "r" (bin_offset),
	      ...
	      [cpuid_offset] /*"i"*/"r" (/*offsetof(rseq_t, cpu_id)*/!opt_ccache_force_cpu_id ? offsetof(struct rseq_abi__workaround, mm_cid) : offsetof(struct rseq_abi__workaround, cpu_id)),  /* $$$  cid addition  $$$ */
