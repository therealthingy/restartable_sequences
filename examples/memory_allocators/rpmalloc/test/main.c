...
test_alloc(void) {
    ...
    if (rpmalloc_usable_size(pointer_add_offset(testptr, 16)) != (wanted_usable_size - 16))
    ...
    for (iloop = 0; iloop < 64; ++iloop) {
        for (ipass = 0; ipass < 8142; ++ipass) {
            ...
            for (icheck = 0; icheck < ipass; ++icheck) {
                ...
                if (addr[icheck] < addr[ipass]) {
                    if (pointer_add_offset(addr[icheck], 500) > addr[ipass])
                        return test_fail("Bad allocation result");
                }
                else if (addr[icheck] > addr[ipass]) {
                    if (pointer_add_offset(addr[ipass], 500) > addr[icheck])
                        return test_fail("Bad allocation result");
                }
            }
        }
    ...
    for (iloop = 0; iloop < 64; ++iloop) {
        for (ipass = 0; ipass < 1024; ++ipass) {
            ...
            for (icheck = 0; icheck < ipass; ++icheck) {
                ...
                if (addr[icheck] < addr[ipass]) {
                    if (pointer_add_offset(addr[icheck], rpmalloc_usable_size(addr[icheck])) > addr[ipass])
                        return test_fail("Invalid pointer inside another block returned from allocation");
                }
                else if (addr[icheck] > addr[ipass]) {
                    if (pointer_add_offset(addr[ipass], rpmalloc_usable_size(addr[ipass])) > addr[icheck])
                        return test_fail("Invalid pointer inside another block returned from allocation");
                }
            }
        }
    ...
    for (iloop = 0; iloop < arg.loops; ++iloop) {
        ...
        for (ipass = 0; ipass < arg.passes; ++ipass) {
            ...
            *(uint32_t*)addr[ipass] = (uint32_t)cursize;
            memcpy(pointer_add_offset(addr[ipass], 4), data, cursize);
            ...
            for (icheck = 0; icheck < ipass; ++icheck) {
                ...
                if (addr[icheck] < addr[ipass]) {
                    if (pointer_add_offset(addr[icheck], *(uint32_t*)addr[icheck] + 4) > addr[ipass]) {
                        ret = test_fail("Invalid pointer inside another block returned from allocation");
                        goto end;
                    }
                }
                else if (addr[icheck] > addr[ipass]) {
                    if (pointer_add_offset(addr[ipass], *(uint32_t*)addr[ipass] + 4) > addr[icheck]) {
                        ret = test_fail("Invalid pointer inside another block returned from allocation");
                        goto end;
                    }
                }
        ...
        for (ipass = 0; ipass < arg.passes; ++ipass) {
            ...
            if (memcmp(pointer_add_offset(addr[ipass], 4), data, cursize)) {
    ...
    for (iloop = 0; iloop < arg.loops; ++iloop) {
        ...
        for (ipass = 0; ipass < arg.passes; ++ipass) {
            ...
            for (icheck = 0; icheck < ipass; ++icheck) {
                ...
                if (addr[icheck] < addr[ipass]) {
                    if (pointer_add_offset(addr[icheck], check_size + sizeof(uint32_t)) > (void*)addr[ipass]) {
                        ret = test_fail("Invalid pointer inside another block returned from allocation");
                        goto end;
                    }
                } else {
                    if (pointer_add_offset(addr[ipass], this_size + sizeof(uint32_t)) > (void*)addr[icheck]) {
                        ret = test_fail("Invalid pointer inside another block returned from allocation");
                        goto end;
                    }
                }
...
static char got_error;

static void
test_error_callback(const char* msg_fmt, ...) {
    //printf("%s\n", msg_fmt);
    (void)sizeof(msg_fmt);
    got_error = 1;
}
...
test_initialize(void) {
    ...
    const int num = CPU_COUNT(&testmask);
